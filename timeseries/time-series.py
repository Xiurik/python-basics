...
class Room:
    def __init__(
        self, Cid, RoomName, StatusStart, StatusEnd, RoomHadPeople, RoomStatus, AccountCid, TimeStamp, MessageId
    ):
        self.Cid = Cid
    ...
    while originalDate.date() < datetime.today().date():
        ...
        if firstRecord is True:
            ...
            while endOfDay is False:
                ...
                data = Room(
                    uuid.uuid4(),
                    rooms[r.randint(0, 9)][1],
                    stStart.strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
                    stEnd.strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
                    roomHadPeople,
                    roomStatus[r.randint(0, 3)][1],
                    accountData[r.randint(0, 19)][0],
                    stEnd.strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
                    "MessageIdData",
                )
                tsSrv.addRow(data)
    ...
    while originalDate.date() < endDate.date():
        ...
        if firstRecord is True:
            ...
            while endOfDay is False:
                ...
                # Start
                data = TimeSeries(
                    room, stStart.strftime("%Y-%m-%dT%H:%M:%S.%fZ"), roomStatus[r.randint(0, 3)][1], "MessageIdData"
                )
                tsSrv.addRow(data)

                # Finish
                data = TimeSeries(
                    room, stEnd.strftime("%Y-%m-%dT%H:%M:%S.%fZ"), roomStatus[r.randint(0, 3)][1], "MessageIdData"
                )
                tsSrv.addRow(data)
...
class TSJson:
    def __init__(
        self, cid, timeStamp, accountCid, roomName, roomStartDateTime, roomEndDateTime, hasPeople, messageId, value
    ):
        self.cid = cid
        self.timeStamp = timeStamp
        self.accountCid = accountCid
...
def generateTimeSeriesJSON():
    ...
    while originalDate.date() < datetime.today().date():
        ...
        while endOfDay is False:
            ...
            srv = TSJson(
                str(uuid.uuid4()),
                str(stEnd.strftime("%Y-%m-%dT%H:%M:%S.%fZ")),
                accountData[r.randint(0, 19)][0],
                rooms[r.randint(0, 9)][1],
                str(stStart.strftime("%Y-%m-%dT%H:%M:%S.%fZ")),
                str(stEnd.strftime("%Y-%m-%dT%H:%M:%S.%fZ")),
                hasPeople,
                "MessageIdData",
                roomStatus[r.randint(0, 3)][1],
    ...
    while originalDate.date() < endDate.date():
        ...
        if firstRecord is True:
            ...
            while endOfDay is False:
                ...
                # Start
                data = XIORoomOccupancy(
                    device, stStart.strftime("%Y-%m-%dT%H:%M:%S.%fZ"), roomStatus[r.randint(0, 3)][1], "MessageIdData"
                )
                tsSrv.addRow(data)
                # Finish
                data = XIORoomOccupancy(
                    device, stEnd.strftime("%Y-%m-%dT%H:%M:%S.%fZ"), roomStatus[r.randint(0, 3)][1], "MessageIdData"
                )
...
def generateXIORoomsCSV():
    ...
    while originalDate.date() < endDate.date():
        ...
        if firstRecord is True:
            ...
            # Start
            data = XIORooms(
                device, stStart.strftime("%Y-%m-%dT%H:%M:%S.%fZ"), rooms[r.randint(0, 9)][1], "MessageIdData"
            )
            tsSrv.addRow(data)
            startDate = stEnd
